import { _decorator, Animation, AudioClip, Collider2D, Component, Contact2DType, IPhysics2DContact } from "cc";
import { AudioMgr } from "db://assets/Scripts/Mgr/AudioMgr";
import { GameManager } from "db://assets/Scripts/Mgr/GameManager";
import { SceneItemManager } from "db://assets/Scripts/Mgr/SceneItemManager";
...
export class Enemy extends Component {
  ...
  onBeginContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
    ...
    this.scheduleOnce(() => {
      if (otherCollider.node && otherCollider.node.isValid) {
        ...
        if (bulletComp) {
          // 使用Tools回收子弹
          Tools.ins.putNodeToPool(Bullet, otherCollider.node);
        }
      }
    }, 0);
  ...
  protected die() {
    ...
    // 记录得分
    GameManager.ins().addScore(this.score);
  }
