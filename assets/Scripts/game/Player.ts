import { _decorator, Animation, AudioClip, Collider, Collider2D, Component, Contact2DType, director, Enum, EventTouch, input, Input, IPhysics2DContact, Node, Prefab } from "cc";
import { AudioMgr } from "db://assets/Scripts/Mgr/AudioMgr";
import { GameManager } from "db://assets/Scripts/Mgr/GameManager";
import { HpUI } from "db://assets/Scripts/UI/HpUI";
...
export class Player extends Component {
  ...
  protected onLoad(): void {
    ...
    // 初始化子弹对象池
    Tools.ins.preloadPoolNode(Bullet, this.bullet1Prefab, 10);
    Tools.ins.preloadPoolNode(Bullet, this.bullet2Prefab, 10);
  }
  ...
  protected onDestroy(): void {
    ...
    // 如果确实需要清理，可以在游戏结束时调用 Tools.ins.clearPool(Bullet)
  }
  // ======================
  // 碰撞处理方法
  // ======================
  ...
  onContactToReward(reward: Reward) {
    ...
    switch (reward.rewardType) {
      ...
      case RewardType.Bomb:
        ...
        GameManager.ins().AddBomb(); // 增加炸弹数量
        break;
    }
  }
  ...
  OneShoot(dt: number) {
    ...
    if (this.shootTimer > this.shootRate) {
      ...
      // 始终使用黄色子弹（单发子弹）
      let bullet1 = Tools.ins.getNodeFromPool(Bullet, this.bullet1Prefab);
  ...
  TwoShoot(dt: number) {
    ...
    if (this.shootTimer > this.shootRate) {
      ...
      // 从对象池管理器获取两颗子弹
      let bullet1 = Tools.ins.getNodeFromPool(Bullet, this.bullet2Prefab);
      let bullet2 = Tools.ins.getNodeFromPool(Bullet, this.bullet2Prefab);
