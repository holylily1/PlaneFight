...
import { Boss } from "../game/Boss"; // 导入Boss类
...
export class SceneItemManager extends Component {
  ...
  onDoubleClick(event) {
    const now = Date.now();
    if (now - this.lastBombTime < 1000) {
      // 1000ms=1秒冷却
      console.log("炸弹冷却中，剩余时间:", 1000 - (now - this.lastBombTime) + "ms");
      return;
    }
    if (!GameManager.ins().isHaveBomb()) return; // 没有炸弹则返回
    ...
    // 使用炸弹
    GameManager.ins().bombNumber--; // 减少炸弹数量
    this.lastBombTime = Date.now(); // 记录炸弹使用时间(毫秒)
    console.log("炸弹使用时间已更新:", this.lastBombTime);
    GameManager.ins().node.emit("onBombChange"); // 通知炸弹数量变化
